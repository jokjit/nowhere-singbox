...
_archive_extractor() {
    local extractor
    for extractor in 7zz 7z 7za; do
...
_install_dependencies() {
    ...
    local archive_pkgs="p7zip-full"
    if command -v apk &>/dev/null || command -v pacman &>/dev/null; then
        archive_pkgs="p7zip"
...
# The target repository ships a single 7z-compatible archive containing the
# Nowhere-enabled Linux ELF. Keep the same staged replacement and rollback
# contract as the upstream installer, but pin the archive itself.
_install_sing_box_custom() {
    local requested_version="${1:-latest}"
    case "$requested_version" in
        latest|"$SINGBOX_FIXED_VERSION") ;;
        *) _error "不允许安装未授权的 sing-box 版本: ${requested_version}"; return 1 ;;
    esac

    case "$(uname -m 2>/dev/null)" in
        x86_64|amd64) ;;
        *)
            _error "Nowhere 核心包目前只提供 Linux x86_64/amd64，当前架构为 $(uname -m 2>/dev/null || printf unknown)。"
            return 1
            ;;
    esac

    local temp_dir archive_path archive_source extractor actual expected extracted_bin
    local staged_bin extracted_version member_names member_count
    ...
    printf '%s\n' "$$" > "${temp_dir}/.active"
    archive_path="${temp_dir}/${SINGBOX_ARCHIVE_NAME}"

    if [ -n "${SINGBOX_ARCHIVE_PATH:-}" ] && [ -s "$SINGBOX_ARCHIVE_PATH" ]; then
        archive_source="$SINGBOX_ARCHIVE_PATH"
    elif [ -s "${SCRIPT_DIR}/${SINGBOX_ARCHIVE_NAME}" ]; then
        archive_source="${SCRIPT_DIR}/${SINGBOX_ARCHIVE_NAME}"
    else
        archive_source="$SINGBOX_ARCHIVE_URL"
    fi
    _info "正在获取包含 Nowhere 协议的 sing-box 核心包..."
    if [[ "$archive_source" == https://* ]]; then
        wget -qO "$archive_path" "$archive_source" || {
            _error "Nowhere 核心包下载失败: $archive_source"
            rm -rf "$temp_dir"
            return 1
        }
    else
        cp -f -- "$archive_source" "$archive_path" || {
            _error "无法读取 Nowhere 核心包: $archive_source"
            rm -rf "$temp_dir"
            return 1
        }
    fi
    ...
    extractor=$(_archive_extractor) || {
        _error "缺少 7z/7zz/7za 解压工具，无法读取 Nowhere 核心包。"
        rm -rf "$temp_dir"
        return 1
    }
    # 7z -slt 先输出归档自身的 Path，再在分隔线后输出真正的成员。
    # 只解析成员区，避免把归档文件名误判成第二个成员。
    member_names=$("$extractor" l -slt "$archive_path" 2>/dev/null | awk -F' = ' '
        /^----------$/ { members=1; next }
        members && $1 == "Path" { print $2 }
    ') || true
    member_count=$(printf '%s\n' "$member_names" | sed '/^$/d' | wc -l | tr -d ' ')
    if [ "$member_count" != "1" ] || [ "$member_names" != "sing-box" ]; then
        _error "Nowhere 核心包内容异常，只允许包含单个 sing-box 文件。"
        rm -rf "$temp_dir"
        return 1
    fi
    if ! "$extractor" x -y -bd "-o${temp_dir}/extracted" "$archive_path" >/dev/null 2>&1; then
        _error "Nowhere 核心包解压失败。"
        rm -rf "$temp_dir"
        return 1
    fi

    extracted_bin=$(find "$temp_dir/extracted" -type f -name sing-box -print -quit 2>/dev/null)
    ...
    if [ "$(od -An -tx1 -N4 "$extracted_bin" 2>/dev/null | tr -d ' \n')" != "7f454c46" ]; then
        _error "Nowhere 核心不是 Linux ELF 可执行文件，已拒绝安装。"
        rm -rf "$temp_dir"
        return 1
    fi
    ...
    rm -f -- "$staged_bin"
    if ! mv -f -- "$extracted_bin" "$staged_bin" && ! cp -- "$extracted_bin" "$staged_bin"; then
        rm -f -- "$staged_bin"
        rm -rf "$temp_dir"
        return 1
    fi
    ...
    rm -rf "$temp_dir"
    _release_install_cache
    SINGBOX_STAGED_VERSION="$extracted_version"
    _success "Nowhere sing-box v${extracted_version} 已安全暂存: ${SINGBOX_BIN}"
}
...
_nowhere_random_password() {
    local password=""
    if [ -x "$SINGBOX_BIN" ]; then
        password=$("$SINGBOX_BIN" generate rand --hex 32 2>/dev/null || true)
    fi
    if [[ ! "$password" =~ ^[0-9A-Fa-f]{64}$ ]]; then
        password=$(openssl rand -hex 32 2>/dev/null || true)
    fi
    [[ "$password" =~ ^[0-9A-Fa-f]{64}$ ]] || return 1
    printf '%s' "$password"
}

_nowhere_network_json() {
    case "$1" in
        tcp) printf '%s' '["tcp"]' ;;
        udp) printf '%s' '["udp"]' ;;
        tcp+udp|udp+tcp|both) printf '%s' '["tcp","udp"]' ;;
        *) return 1 ;;
    esac
}

_nowhere_carrier_for_network() {
    case "$1" in
        udp) printf '%s' udp ;;
        tcp+udp|udp+tcp|both) printf '%s' mix ;;
        *) printf '%s' tcp ;;
    esac
}
...
_add_nowhere() {
    local node_ip="${server_ip}" client_server_addr port network_choice network network_json carrier port_proto
    local sni password tag cert_path key_path pin name link_ip inbound_json
    if [ "$BATCH_MODE" = "true" ]; then
        ...
        _info "--- Nowhere 设置向导 ---"
        read -r -p "请输入客户端连接地址 (默认: ${server_ip}): " client_server_addr
        client_server_addr=${client_server_addr:-$server_ip}
        client_server_addr=$(_normalize_client_server "$client_server_addr")
        [[ -n "$client_server_addr" && "$client_server_addr" != *[[:space:]/?#]* && "$client_server_addr" != *"://" ]] || {
            _error "连接地址格式无效，请只输入 IP 或域名。"; return 1;
        }
        while true; do
            read -r -p "请输入监听端口: " port
            [[ "$port" =~ ^[0-9]+$ && "$port" -ge 1 && "$port" -le 65535 ]] || { _error "端口必须是 1-65535 的整数。"; continue; }
            read -r -p "请选择网络 [1] TCP+UDP [2] TCP [3] UDP (默认1): " network_choice
            case "${network_choice:-1}" in
                1) network_choice=tcp+udp; break ;;
                2) network_choice=tcp; break ;;
                3) network_choice=udp; break ;;
                *) _error "请选择 1、2 或 3。" ;;
            esac

        done
        local port_proto
        port_proto="$network_choice"
        ...
        [ -n "$password" ] || { _error "密码生成失败。"; return 1; }
        read -r -p "请输入节点名称 (默认 Nowhere-${port}): " name
        name=${name:-Nowhere-${port}}
    fi

    network_json=$(_nowhere_network_json "$network_choice") || { _error "Nowhere 网络设置无效。"; return 1; }
    network=$(printf '%s' "$network_choice")
    carrier=${carrier:-$(_nowhere_carrier_for_network "$network_choice")}
    [[ "$sni" =~ ^[A-Za-z0-9][A-Za-z0-9._-]*$ ]] || { _error "SNI 只能包含字母、数字、点、下划线和连字符。"; return 1; }
    port_proto="$network_choice"
    [[ "$port" =~ ^[0-9]+$ && "$port" -ge 1 && "$port" -le 65535 ]] || { _error "端口必须是 1-65535 的整数。"; return 1; }
    if [ "$BATCH_MODE" = "true" ] && _check_port_conflict "$port" "$port_proto" "true"; then
        _error "Nowhere 端口 ${port} 已被占用。"
        return 1
    fi
    client_server_addr=$(_normalize_client_server "$client_server_addr")
    [ -n "$client_server_addr" ] || { _error "连接地址不能为空。"; return 1; }
    [ -n "$password" ] || password=$(_nowhere_random_password) || { _error "密码生成失败。"; return 1; }
    ...
    pin=$(_cert_sha256_hex "$cert_path") || { _error "无法计算 Nowhere 证书指纹。"; return 1; }
    [[ "$pin" =~ ^[0-9a-f]{64}$ ]] || { _error "Nowhere 证书指纹格式异常。"; return 1; }

    inbound_json=$(jq -n --arg tag "$tag" --arg p "$port" --arg pw "$password" --arg cert "$cert_path" --arg key "$key_path" --argjson network "$network_json" \
        '{type:"nowhere",tag:$tag,listen:"::",listen_port:($p|tonumber),password:$pw,network:$network,tls:{enabled:true,alpn:["now/1"],min_version:"1.3",max_version:"1.3",certificate_path:$cert,key_path:$key}}') || return 1
    _atomic_modify_json "$CONFIG_FILE" '.inbounds += [$node] | .inbounds |= unique_by(.tag)' --argjson node "$inbound_json" || return 1
    _atomic_modify_json "$METADATA_FILE" '. + {($tag): ((.[$tag] // {}) + {name:$name,owner:"singbox-main",variant:"nowhere",clientServer:$server,serverName:$sni,certificatePin:$pin,network:$network,carrier:$carrier,yaml:false})}' \
        --arg tag "$tag" --arg name "$name" --arg server "$client_server_addr" --arg sni "$sni" --arg pin "$pin" --arg network "$network" --arg carrier "$carrier" || {
        _rollback_main_node_creation "$tag"
        return 1
    }

    link_ip="$client_server_addr"
    [[ "$link_ip" == *":"* && "$link_ip" != "["* ]] && link_ip="[$link_ip]"
    _success "Nowhere 节点 [${name}] 添加成功。"
    _show_node_link "nowhere" "$name" "$link_ip" "$port" "$tag" "$password" "$sni" "$pin" "$carrier" "$network_json" || return 1
}
...
_show_node_link() {
    ...
    case "$type" in
        ...
        "nowhere")
            ...
            echo ""
            echo -e "${YELLOW}════════════════ Nowhere 客户端 JSON ════════════════${NC}"
            printf '%s\n' "$client_json" | jq .
            echo -e "${YELLOW}════════════════════════════════════════════════════${NC}"
            _info "客户端必须使用包含 Nowhere 的 sing-box 核心；Clash/Mihomo 不支持此协议。"
            ;;
    esac
    ...
    if [ -n "$client_json" ] && [ -n "$tag" ] && [ "$tag" != "null" ] && [[ "$tag" != argo-* ]]; then
        _atomic_modify_json "$METADATA_FILE" '. + {($tag): ((.[$tag] // {}) + {clientConfig:$config,yaml:false})}' \
            --arg tag "$tag" --argjson config "$client_json" || return 1
    fi
...
_view_nodes() {
    ...
    while IFS= read -r node; do
        ...
        if [ -n "$url" ] && [ "$url" != "null" ] && [ "$type" != "nowhere" ]; then
            : # 直接使用持久化链接
        else
            case "$type" in
            "vless")
            ...
            "anytls")
                ...
                ;;
            "nowhere")
                local nowhere_json nowhere_pin nowhere_sni nowhere_carrier nowhere_server nowhere_password
                nowhere_json=$(jq -c --arg t "$tag" '.[$t].clientConfig // empty' "$METADATA_FILE" 2>/dev/null)
                if [ -n "$nowhere_json" ] && [ "$nowhere_json" != "null" ]; then
                    ...
                    nowhere_password=$(printf '%s' "$node" | jq -r '.password // empty')
                    nowhere_json=$(_nowhere_json "$nowhere_server" "$port" "$nowhere_password" "$nowhere_sni" "$nowhere_pin" "$nowhere_carrier" 2>/dev/null || true)
                fi
                if [ -n "$nowhere_json" ] && [ "$nowhere_json" != "null" ]; then
                    [ -n "$nowhere_server" ] || nowhere_server="$display_ip"
                    [ -n "$nowhere_password" ] || nowhere_password=$(printf '%s' "$node" | jq -r '.password // empty')
                    local nowhere_link_server="$nowhere_server"
                    [[ "$nowhere_link_server" == *":"* && "$nowhere_link_server" != "["* ]] && nowhere_link_server="[${nowhere_link_server}]"
                    url="vector://$(_url_encode "$nowhere_password")@${nowhere_link_server}:${port}?up=${nowhere_carrier}&down=${nowhere_carrier}&sni=$(_url_encode "$nowhere_sni")"
                    [ -n "$nowhere_pin" ] && url="${url}&pin=${nowhere_pin}"
...
_refresh_modified_node_artifacts() {
    ...
    local yaml_enabled
    yaml_enabled=$(jq -r --arg tag "$tag" '.[$tag] | if has("yaml") then (.yaml | tostring) else "true" end' "$METADATA_FILE" 2>/dev/null)
    ...
    local uuid password username method sni public_key short_id cert_path key_path skip_verify pin network carrier
    ...
    case "$variant" in
        nowhere)
            ...
            if [ -z "$pin" ]; then
                pin=$(jq -r --arg tag "$tag" '.[$tag].certificatePin // empty' "$METADATA_FILE")
            fi
            network=$(printf '%s' "$node" | jq -r '(.network // ["tcp","udp"]) | if type == "array" then join("+") else . end')
            carrier=$(jq -r --arg tag "$tag" '.[$tag].carrier // empty' "$METADATA_FILE")
            case "$carrier" in tcp|udp|mix) ;; *) carrier=$(_nowhere_carrier_for_network "$network") ;; esac
            _atomic_modify_json "$METADATA_FILE" '
                .[$tag] |= (. + {
                    serverName:$sni,
                    ...
                    network:$network,
                    carrier:$carrier
                } | if $pin != "" then .certificatePin = $pin | .certSha256 = $pin else del(.certificatePin, .certSha256) end)
            ' --arg tag "$tag" --arg sni "$sni" --arg cert "$cert_path" --arg key "$key_path" --arg network "$network" --arg carrier "$carrier" --arg pin "$pin" || return 1
            _show_node_link "$variant" "$name" "$client_server" "$port" "$tag" "$password" "$sni" "$pin" "$carrier" || return 1
            ;;
        vless-reality)
...
_modify_port() (
    ...
    if [ "$type_to_modify" = "nowhere" ]; then
        local nowhere_network_current
        nowhere_network_current=$(jq -r --arg tag "$tag_to_modify" '.inbounds[]? | select(.tag == $tag) | (.network // ["tcp","udp"]) | if type == "array" then join("+") else . end' "$CONFIG_FILE" 2>/dev/null)
        case "$nowhere_network_current" in
            udp) listen_proto="udp" ;;
            tcp) listen_proto="tcp" ;;
    ...
    if [ "$proposed_new_tag" != "$tag_to_modify" ] \
        && [[ "$type_to_modify" == "hysteria2" || "$type_to_modify" == "tuic" || "$type_to_modify" == "anytls" || "$type_to_modify" == "nowhere" ]]; then
    ...
    if [ "$new_tag" != "$tag_to_modify" ]; then
        ...
        if [ "$type_to_modify" == "hysteria2" ] || [ "$type_to_modify" == "tuic" ] || [ "$type_to_modify" == "anytls" ] || [ "$type_to_modify" == "nowhere" ]; then
        ...
        identity)
            case "$variant" in
                ...
                shadowsocks-shadowtls)
                    ...
                    ;;
                vless-ws-tls|vless-grpc-tls|trojan-ws-tls|anytls|hysteria2|tuic|nowhere)
...
_modify_node() {
    ...
    case "$variant" in
        ...
        vless-ws-tls|vless-grpc-tls|trojan-ws-tls|anytls|hysteria2|tuic|nowhere) echo "  5) 修改 SNI 与证书" ;;
    esac
    case "$variant" in
        ...
        4)
            case "$variant" in
                ...
                trojan-ws-tls|hysteria2|anytls|any-reality)
                    ...
                    ;;
                nowhere)
                    read -r -s -p "请输入新密码 (回车随机生成): " value; echo ""; value=${value:-$(_nowhere_random_password)}
                    ;;
                tuic)
                    read -r -p "请输入新 UUID (回车随机生成): " value; value=${value:-$(${SINGBOX_BIN} generate uuid)}
                    [[ "$value" =~ ^[0-9A-Fa-f-]{36}$ ]] || { _error "UUID 格式无效。"; return 1; }
...
# [安装/更新 Sing-box 核心] — 固定 Nowhere 核心包
_install_or_update_singbox() {
    local current_ver="未安装" choice confirm
    if [ -f "${SINGBOX_BIN}" ]; then
        current_ver=$(${SINGBOX_BIN} version 2>/dev/null | head -n1 | sed 's/^sing-box version[[:space:]]*//')
    ...
    if _singbox_core_is_locked; then
        echo -e "  版本策略: ${YELLOW}固定 ${SINGBOX_FIXED_VERSION}（禁止升级）${NC}"
        local locked_version
        locked_version=$(head -n 1 "$SINGBOX_CORE_LOCK_FILE" 2>/dev/null || true)
        if [ "$locked_version" != "$SINGBOX_FIXED_VERSION" ]; then
            _warn "检测到固定锁文件内容异常，请选择 [2] 恢复固定版。"
        fi
    else
        echo -e "  版本策略: ${GREEN}使用 Nowhere 固定核心包${NC}"
    fi
    echo ""
    echo -e "    ${GREEN}[1]${NC} 安装/更新 Nowhere 核心包"
    echo -e "    ${GREEN}[2]${NC} 安装并锁定 ${SINGBOX_FIXED_VERSION}（安装后禁止升级）"
    echo -e "    ${YELLOW}[0]${NC} 返回主菜单"
    echo ""
    ...
    case "$choice" in
        1)
            if _singbox_core_is_locked; then
                _error "当前核心已固定为 ${SINGBOX_FIXED_VERSION}，不允许升级。"
        ...
        2)
            if ! _singbox_core_is_locked; then
                _warn "安装完成后，脚本将锁定 ${SINGBOX_FIXED_VERSION}，后续菜单不再允许核心升级。"
...
_do_update_singbox() {
    ...
    _commit_singbox_binary
    if [ "$install_target" = "$SINGBOX_FIXED_VERSION" ]; then
        _success "[主] 服务已使用固定核心 ${SINGBOX_FIXED_VERSION} 启动，版本升级锁已生效。"
    else
        _success "[主] 服务已使用 Nowhere 核心 v${SINGBOX_STAGED_VERSION:-未知} 启动，更新已提交。"
    fi
}
...
_batch_create_nodes() {
    ...
    local has_hy2=false
    local has_ss=false
    local has_nowhere=false
    local nowhere_network="tcp+udp"
    ...
    for pid in $proto_ids; do
        ...
        [[ "$pid" =~ ^(1|4|5|6|7|11)$ ]] && has_sni_req=true
        [[ "$pid" == "6" ]] && has_hy2=true
        [[ "$pid" == "11" ]] && has_nowhere=true
    done
    ...
    if [ "$has_nowhere" = true ]; then
        read -r -p "Nowhere 网络 [1] TCP+UDP [2] TCP [3] UDP (默认1): " nowhere_choice
        case "${nowhere_choice:-1}" in
            1) nowhere_network="tcp+udp" ;;
            2) nowhere_network="tcp" ;;
            3) nowhere_network="udp" ;;
            *) _error "Nowhere 网络选择无效。"; return 1 ;;
        esac
        export BATCH_NOWHERE_NETWORK="$nowhere_network"
    fi
    ...
    for planned_pid in $proto_ids; do
        case "$planned_pid" in
            ...
            11) planned_protocols+=("$nowhere_network") ;;
            8)
...
# 解析命令行参数。测试或其他脚本 source 本文件时只加载函数，避免意外进入
# root 检查、依赖安装和交互菜单。
if [ "${SINGBOXLITE_TEST_SOURCE_ONLY:-0}" != "1" ]; then
    while [[ $# -gt 0 ]]; do
        case "$1" in
            keepalive)
                _argo_keepalive
